...
# that represents the electric circuit that is being modeled
        if filename != "":
            ...
            self.text.Command = "New EnergyMeter.Feeder Line.632670 1"
...
# also populates the branch currents and bus voltages
    ...
    def get_temp_results(self):
        bus_v = Bus(self.circuit).V
        branch = Branch(self.circuit)
        branch_i = branch.I
        branch_v = branch.V
        return [bus_v, branch_i, branch_v]
...
# in a text window
    def print_branch_info(self, event):
        ...
        print("voltages (120-V base): ", np.around(abs(self.branch.Vto[ind]) / self.branch.kvbase[ind] * 120 / 1000, 1))
...
if __name__ == '__main__':
    d = DSS(r"C:\Program Files\OpenDSS\IEEETestCases\13Bus\IEEE13Nodeckt.dss")
    # d.plots()
    res1 = d.get_temp_results()
    d.add_command("Transformer.Reg1.Taps=[1.0 1.0625]")
    d.add_command("Transformer.Reg2.Taps=[1.0 1.0500]")
    d.add_command("Transformer.Reg3.Taps=[1.0 1.06875]")
    d.add_command("Set Controlmode=OFF")
    d.solve()
    res2 = d.get_temp_results()

    # [bus_v, branch_i, branch_v]
    diff_bus_v = res1[0] - res2[0]
    diff_branch_i = res1[1] - res2[1]
    diff_branch_v = res1[2] - res2[2]
    np.abs(diff_bus_v)
