...
with open("supervised_learning.json") as par:
    ...
    model_path = pars['root'] + pars['model_dir']
    window = pars['window']
    shift = pars['shift'] % window
    data = pd.read_csv(data_path + "fabricated_w%s_d%s.csv" % (window, shift))
    ...
    # pre-processing: scaler, feature-selector
    ...
    if cross_validation:
        kf = KFold(n_splits=pars['fold_number'], shuffle=True)
        ...
        path_saving = model_path + '/shift_w%s_d%s.pkl' % (window, shift)
